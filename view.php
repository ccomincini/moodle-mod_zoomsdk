...
echo html_writer::tag('p', html_writer::tag('strong', get_string('starttime', 'mod_zoomsdk') . ': ') . userdate($zoomsdk->starttime));
echo html_writer::tag('p', html_writer::tag('strong', get_string('duration', 'mod_zoomsdk') . ': ') . format_time($zoomsdk->duration));
echo html_writer::tag('p', html_writer::tag('strong', get_string('meetingid', 'mod_zoomsdk') . ': ') . $zoomsdk->meetingid);
echo $OUTPUT->box_end();
...
function initZoomSDK(config) {
    ...
    resizeZoomContainer(); // Dinamico all'avvio
    ZoomMtg.preLoadWasm();
